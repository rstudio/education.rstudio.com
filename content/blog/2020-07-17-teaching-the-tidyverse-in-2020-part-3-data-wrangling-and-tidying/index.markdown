...
A common task in data analysis is going from wide data (e.g. where you recorded repeated measurements on a subject in consecutive columns) to long data, or vice versa. We'll stick with penguins in this post, but make up some (realistic) data. Suppose we have 4 penguins in our sample, and we take measurements of their body mass. We measure each penguin 3 times.
